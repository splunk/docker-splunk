...
Splunk's Docker container has several functions that can be configured. These options are specified by either supplying a `defaults.ym`l or
...
|---|---|:---:|:---:|:---:|
| SPLUNK_BUILD_URL | URL to Splunk build where we can fetch a Splunk build to install | | | |
| SPLUNK_DEFAULTS_URL | Defaults.yml URL | | |  |
| SPLUNK_UPGRADE | If this is True, we won’t run any provisioning after installation. Use this to upgrade and redeploy containers with a newer version of Splunk. | | | |
| SPLUNK_ROLE | Specify the container’s current Splunk Enterprise role. Supported Roles: splunk_standalone, splunk_indexer, splunk_deployer, splunk_search_head, etc. | | yes | yes |
| DEBUG | Print Ansible vars to stdout (supports Docker logging) | | | |
...
| SPLUNK_LICENSE_URI | URI we can fetch a Splunk Enterprise license. This can be a local path or a remote URL. | | | |
| SPLUNK_STANDALONE_URL | List of all Splunk Enterprise standalone hosts (network alias) separated by comma | | | |
| SPLUNK_SEARCH_HEAD_URL | List of all Splunk Enterprise search head hosts (network alias) separated by comma |  | yes | yes |
| SPLUNK_INDEXER_URL| List of all Splunk Enterprise indexer hosts (network alias) separated by comma | | yes | yes |
| SPLUNK_HEAVY_FORWARDER_URL | List of all Splunk Enterprise heavy forwarder hosts (network alias) separated by comma | | | |
| SPLUNK_DEPLOYER_URL | One Splunk Enterprise deployer host (network alias) | | yes | |
| SPLUNK_CLUSTER_MASTER_URL | One Splunk Enterprise cluster master host (network alias) | | | yes |
| SPLUNK_SEARCH_HEAD_CAPTAIN_URL | One Splunk Enterprise search head host (network alias). Passing this ENV variable will enable search head clustering. | | yes | |
| SPLUNK_PASSWORD | Default password of the admin user| yes  | yes | yes |
| SPLUNK_HEC_TOKEN | HEC (HTTP Event Collector) Token when enabled | | | |
| SPLUNK_SHC_SECRET | Search Head Clustering Shared secret | | yes | |
| SPLUNK_IDXC_SECRET | Indexer Clustering Shared Secret | | | yes |
| NO_HEALTHCHECK | Disable the Splunk healthcheck script | | | yes |

#### Valid Universal Forwarder Environment Variables
|Environment Variable Name| Description | Required for Standalone| Required for Search Head Clustering | Required for Index Clustering |
|---|---|:---:|:---:|:---:|
| SPLUNK_DEPLOYMENT_SERVER | One Splunk host (network alias) that we use as a deployment server. (http://docs.Splunk.com/Documentation/Splunk/latest/Updating/Configuredeploymentclients) | | | |
| SPLUNK_ADD | List of items to add to monitoring separated by comma. Example, SPLUNK_ADD=udp 1514,monitor /var/log/*. This will monitor udp 1514 port and /var/log/* files. | | | |
| SPLUNK_BEFORE_START_CMD | List of commands to run before Splunk starts separated by comma. Ansible will run “{{splunk.exec}} {{item}}”. | | | |
| SPLUNK_CMD | List of commands to run after Splunk starts separated by comma. Ansible will run “{{splunk.exec}} {{item}}”. | | | |
| DOCKER_MONITORING | True or False. This will install Docker monitoring apps. | | | |
...
|hec_port| Default HEC Input Port | splunk | 8088 | | | |
|hec_disabled| Enable / Disable HEC | splunk | 0 | | | |
|hec_enableSSL| Force HEC to use encryption | splunk | 1 | | | |
|hec_token| Token to enable for HEC inputs | splunk | **none** | | | |
...
|Variable Name| Description | Parent Object | Default Value | Required for Standalone| Required for Search Head Clustering | Required for Index Clustering |
|---|---|:---:|:---:|:---:|:---:|:---:|
|default| Normal apps for standalone instances will be installed in this location | splunk.app_paths | **none** | | | |
|shc| Apps for search head cluster instances will be installed in this location (usually only done on the deployer)| splunk.app_paths | **none** | | | |
|idxc| Apps for index cluster instances will be installed in this location (usually only done on the cluster master)| splunk.app_paths | **none** | | | |
|httpinput| App to use and configure when setting up HEC based instances.| splunk.app_paths | **none** | | | |
...
|Variable Name| Description | Parent Object | Default Value | Required for Standalone| Required for Search Head Clustering | Required for Index Clustering |
|---|---|:---:|:---:|:---:|:---:|:---:|
|enable| Instructs the container to create a search head cluster | splunk.shc | false| | yes | |
|secret| A secret phrase to use for all SHC communication and binding. Please note, once set this can not be changed without rebuilding the cluster. | splunk.shc | **none** | | yes | |
|replication_factor| Consult docs.splunk.com for valid settings for your use case | splunk.shc | 3| | yes | |
|replication_port| Default port for the SHC to communicate on | splunk.shc | 4001| | yes | |
...
|Variable Name| Description | Parent Object | Default Value | Required for Standalone| Required for Search Head Clustering | Required for Index Clustering |
|---|---|:---:|:---:|:---:|:---:|:---:|
| secret | Secret used for transmission between the cluster master and indexers | splunk.idxc | **none** | | | yes |
| search_factor | Search factor to be used for search artifacts | splunk.idxc | 2 | | | yes |
| replication_factor | Bucket replication factor used between index peers | splunk.idxc | 3 | | | yes |
| replication_port | Bucket replication Port between index peers | splunk.idxc | 9887 | | | yes |
...
For more information about these roles, refer to [Splunk Splexicon](http://docs.splunk.com/splexicon).
